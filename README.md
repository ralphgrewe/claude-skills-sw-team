...
| Implementation sub-agents | haiku by default (per issue) | Chosen from the `## Model recommendation` section the product manager writes into each issue. Haiku unless the issue names concrete evidence it would fail (then sonnet); defaults to haiku if the section is absent. Opus is never dispatched upfront — escalation only. |
| Design-review sub-agent | sonnet | Reviews every verified commit for design/architecture smells, so haiku-first routing doesn't quietly accumulate design debt. |

The skills only control the sub-agent tier: the model for the product manager and manager is whatever the session invoking the skill runs on (set via `/model`). If verification or the design review of an implementation fails, `sw-team-manager` mandatorily escalates that issue one tier up (haiku → sonnet → opus) before involving you, adds the "Claude-Model-Escalation" label, and records the escalation reason as an issue comment — querying that label pulls up all escalated issues, and those comments are the data for refining the haiku/sonnet criteria over time.
